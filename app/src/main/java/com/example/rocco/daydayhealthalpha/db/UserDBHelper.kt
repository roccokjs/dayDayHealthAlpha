...
class UsersDBHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun insertUser(user: User): Boolean {
        ...
        values.put(DBContract.UserEntry.COLUMN_HEIGHT, user.height)
        values.put(DBContract.UserEntry.COLUMN_WEIGHT, user.weight)
        values.put(DBContract.UserEntry.COLUMN_SEX, user.sex)
    ...
    fun readUser(userid: String): ArrayList<User> {
        ...
        if (cursor!!.moveToFirst()) {
            while (cursor.isAfterLast == false) {
                ...
                height = cursor.getInt(cursor.getColumnIndex(DBContract.UserEntry.COLUMN_HEIGHT))
                weight = cursor.getInt(cursor.getColumnIndex(DBContract.UserEntry.COLUMN_WEIGHT))
                sex = cursor.getInt(cursor.getColumnIndex(DBContract.UserEntry.COLUMN_SEX))

                users.add(User(userid, name, age, height, weight, sex))
                cursor.moveToNext()
            }
        }
        return users
    ...
    companion object {
        ...
        private val SQL_CREATE_ENTRIES =
                "CREATE TABLE " + DBContract.UserEntry.TABLE_NAME + " (" +
                        ...
                        DBContract.UserEntry.COLUMN_HEIGHT + " Text," +
                        DBContract.UserEntry.COLUMN_WEIGHT + " Text," +
                        DBContract.UserEntry.COLUMN_SEX + " Text)"
